...
impl f64ad {
    ...
    fn transfer_to_other_global_computation_graph(&mut self, name: Option<&str>, idx: Option<usize>) {
        match self {
            ...
            _ => { panic!("cannot transfer an f64ad_ that is not an f64ad_var.") }
        }
    }
    fn clone_computation_graph_to_other_global_computation_graph(&self, name: Option<&str>, idx: Option<usize>) {
        match self {
            ...
            _ => { panic!("cannot clone computation graph of an f64ad_ that is not an f64ad_var.") }
        }
    }
...
#[derive(Clone, Debug)]
pub (crate) struct ComputationGraph_ {
    id: usize,
    nodes: Vec<F64ADNode>,
    mode: ComputationGraphMode,
}
...
/*
fn f64ad_jacobian_internal_parallel(inputs: &[f64ad_], outputs: &JacobianOutput, add_to_computation_graph: bool) -> JacobianOutput {
